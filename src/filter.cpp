...
/* static */
bool FilterCriteria::convert_extract_filter(const std::string& extract_filter,
    std::string& output_pcap_filter)
{
    ...
    case 2:
        ...
        if (ip_port.size() != 4) {
            printf_error("Expected an IP address and a port number separated by a "
                         "colon; found: %s and %s invalid IP:port strings.\n",
    ...
    case 4:
        ...
        break;
    default:
        printf_error("Expected space-separated IP:port strings instead of %s\n",
...
static bool
apply_filter_on_inner_ip_frame(const Packet& pkt, unsigned int inner_ip_offset,
    unsigned int ipver,
    unsigned int len_after_inner_ip_start,
    const struct bpf_program* gtpu_filter)
{
    ...
    switch (ipver) {
    case 4:
        fake_ehdr->ether_type = htons(ETH_P_IP); // erase any layer (like VLAN)
            // possibly present in orig packet
        break;

    case 6:
        fake_ehdr->ether_type = htons(ETH_P_IPV6); // erase any layer (like VLAN) possibly present in
            // orig packet
        break;
...
bool FilterCriteria::is_matching(const Packet& pkt, bool* is_gtpu) // will do a logical OR of all filters set
{
    ...
    if (UNLIKELY(m_gtpu_filter_set)) {
        ...
        ParserRetCode_t errcode = get_gtpu_inner_ip_start_offset(
            pkt, &offset, &ipver, &len_after_inner_ip_start, NULL, NULL);
        if (errcode == GPRC_VALID_PKT) {
            ...
            if (offset > 0 && len_after_inner_ip_start > 0) {
                ...
                if (apply_filter_on_inner_ip_frame(
                        pkt, offset, ipver, len_after_inner_ip_start, &m_gtpu_filter))
                    return true; // useless to proceed!
            }
        }
    }
            ...
            if (entry != m_valid_tcp_firstpass_flows.end()) {
                ...
                case TCP_FILTER_CONN_HAVING_SYN:
                    if (entry->second >= FLOW_FOUND_SYN_AND_SYNACK)
                        return true; // this TCP packet belongs to a connection that in the
                            // 1st pass was detected as having at least one SYN
                    break;
                case TCP_FILTER_CONN_HAVING_FULL_3WAY_HANDSHAKE:
                    if (entry->second >= FLOW_FOUND_SYN_AND_SYNACK_AND_ACK)
                        return true; // this TCP packet belongs to a connection that in the
                            // 1st pass was detected as having the full 3way
                            // handshake
                    break;
...
bool FilterCriteria::post_filtering(unsigned long nloaded)
{
    if (m_gtpu_filter_set) {
        // in this case, the GTPu parser was run and we have a stat about how many
        // packets are GTPu
        printf_verbose(
            "%luM packets (%lu packets) loaded from the input PCAP are GTPu "
            "packets (%.1f%%).\n",
            m_num_gtpu_pkts / MILLION, m_num_gtpu_pkts,
            (double)(100.0 * (double)(m_num_gtpu_pkts) / (double)(nloaded)));
    }
